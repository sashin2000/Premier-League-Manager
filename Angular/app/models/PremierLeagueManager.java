...
/*
* This is class Premiere League Manager
...
public class PremierLeagueManager implements PremierLeague, Serializable {
    ...
    /* As there is only one premiere league manager for the whole application,
    * there is not need to create multiple objects for this class.
    * So I have used singleton design pattern for this class
    */
    ...
    * - Validations -
    *
    * Team already exist in the database cannot be created as a new team
    * Input other than given 3 options, is ignored
    *
    * - Errors handled-
    *
    * Handling input mismatches(club type can be only an integer)
    */
    @Override
    public void createNewClub(int clubType) {
        ...
        if(isAddedClubDuplicated(clubName)){
            return;
        }
    ...
    // This method is used for deleting a club

    /*
     * - Validations -
     *
     * Team that isn't exist in the database cannot be deleted
     */
    @Override
    ...
    // This method is used for displaying the given statistics for a given club

    /*
    * -Validations-
    * Statistics of the team that isn't exist in the database cannot be displayed
    */
    ...
    public void addPlayedMatch() {
        ...
        if(isAddedMatchValid(team1, team2)){
    ...
    public void addToPlayedMatchList(String team1, int team1Score,String team2, int team2Score, Date matchDate) {
        ...
        // If the matchDate does not exists as a key in hashmap matches, create a new key with the new match date and an array list
        if(!this.matches.containsKey(matchDate)){
            this.matches.put(matchDate, new ArrayList<PlayedMatch>());
        }

        // Get the array list relevant to the match date and add the played match in it
        this.matches.get(matchDate).add(playedMatch);
    ...
    // This method is used for print each record of the premiere league table.
    ...
    // This method is used for check the validity of the added match by the user
    private boolean isAddedMatchValid(String team1Name, String team2Name){
        FootballClub team1 = null;
        FootballClub team2 = null;

        // Checks whether the added team name is registered on the database
        for (FootballClub club : clubList) {
            if(team1Name.toLowerCase().equals(club.getClubName().toLowerCase())){
                team1 = club;
            }

            if(team2Name.toLowerCase().equals(club.getClubName().toLowerCase())){
                team2 = club;
            }
        }

        // If the entered name of team1 is not registered display the message and return false
        if(team1 == null){
            System.out.println("\nThere is no team registered with the name " + team1Name + "\n");
            return false;
        }

        // If the entered name of team2 is not registered display the message and return false
        if(team2 == null){
            System.out.println("\nThere is no team registered with the name " + team2Name + "\n");
            return false;
        }

        // If team1 and team2 are equal, display the message anf return false
        if(team1Name.toLowerCase().equals(team2Name.toLowerCase())){
            System.out.println("\nPlease enter two different teams.\n");
            return false;
        }

        // Checks whether the two teams entered are in same club categories
        if(team1 instanceof SchoolFootballClub && team2 instanceof SchoolFootballClub){
            return true;
        }else if(team1 instanceof UniversityFootballClub && team2 instanceof UniversityFootballClub){
            return true;
        }else if(!(team1 instanceof SchoolFootballClub || team1 instanceof UniversityFootballClub) && !(team2 instanceof SchoolFootballClub || team2 instanceof UniversityFootballClub)){
            return true;
        }else {
            System.out.println("\nCannot add played matches with mixed club categories.\n");
            return false;
        }
    }

    // When creating a new club, if an entered club name is already exists in the database, this method is used to validate it
    private boolean isAddedClubDuplicated(String clubName){
        for (FootballClub club : clubList) {
            if(clubName.toLowerCase().equals(club.getClubName().toLowerCase())){
                System.out.println("\nClub named " + clubName + " has been already registered.\n");
                return true;
            }
        }
        return false;
